...
ClusterIcon.prototype.onAdd = function() {
  ...
  var i, bounds = new google.maps.LatLngBounds(this.center_, this.center_),
  ...
  if (marker.isAdded = !0, this.markers_.push(marker), mCount = this.markers_.length, mz = this.markerClusterer_.getMaxZoom(), null !== mz && this.map_.getZoom() > mz) marker.getMap() !== this.map_ && marker.setMap(this.map_);
  else if (mCount < this.minClusterSize_) marker.getMap() !== this.map_ && marker.setMap(this.map_);
  ...
  if (!(this.styles_.length > 0))
    for (i = 0; i < this.imageSizes_.length; i++) size = this.imageSizes_[i], this.styles_.push({
      url: this.imagePath_ + (i + 1) + "." + this.imageExtension_,
      height: size,
      width: size
    })
}, MarkerClusterer.prototype.fitMapToMarkers = function() {
  var i, markers = this.getMarkers(),
  ...
  var i, d, cluster, center, distance = 4e4,
    clusterToAddTo = null;
  ...
  if (this.ready_) {
    ...
    if (iLast < this.markers_.length) this.timerRefStatic = setTimeout(function() {
      ...
      for (delete this.timerRefStatic, google.maps.event.trigger(this, "clusteringend", this), i = 0; i < this.clusters_.length; i++) this.clusters_[i].updateIcon_()
  }
...
(function() {
  ...
  var data = [{
    ...
    lat: 37.4419,
    lon: -122.1419,
    title: 'Marker same location 3'
  }, {
    lat: 37.4419,
    lon: -122.1419,
    title: 'Marker same location 4'
  }, {
    lat: 37.4419,
    lon: -122.1419,
    title: 'Marker same location 5'
  }, {
    lat: 37.4419,
    lon: -122.1419,
    title: 'Marker same location 6'
  }, {
    lat: 37.4419,
    lon: -122.1419,
    title: 'Marker same location 7'
  }, {
    lat: 37.4419,
    lon: -122.1419,
    title: 'Marker same location 8'
  }, {
    lat: 37.4419,
    lon: -122.1419,
    title: 'Marker same location 9'
  }];
